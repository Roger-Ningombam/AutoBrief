...
function exportToPDF() {
    ...
    // 1. Check if there is content to export
    ...
    // 2. Build the complete HTML for the PDF as a string, just like your original code
    // This ensures all styles are included correctly.
    const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                body { font-family: 'Times New Roman', serif; max-width: 800px; margin: 0 auto; padding: 40px 20px; line-height: 1.6; color: #333; }
                h1 { color: #8b7355; border-bottom: 2px solid #8b7355; padding-bottom: 10px; margin-bottom: 30px; text-align: center; }
                .generated-by { text-align: center; margin-bottom: 30px; color: #666; font-style: italic; }
                .content { text-align: justify; font-size: 14px; }
                b, strong { color: #8b7355; } /* Makes bolded headings from AI match your style */
        ...
        <body>
            ...
            <div class="content">${readingContent.innerHTML}</div>
            <div class="footer">Generated on ${new Date().toLocaleDateString()} | AutoBrief.com</div>
        </body>
        </html>
    `;

    // 3. Set the options for the PDF file
    const opt = {
      margin:       0.5,
      filename:     `${bookTitle.replace(/ /g, '_') || 'AutoBrief_Summary'}.pdf`,
      image:        { type: 'jpeg', quality: 0.98 },
      html2canvas:  { scale: 2 },
      jsPDF:        { unit: 'in', format: 'letter', orientation: 'portrait' }
    };

    // 4. Use html2pdf.js to convert the HTML STRING and trigger the download
