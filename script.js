...
function exportToPDF() {
    ...
    console.log("Starting PDF generation..."); // Debugging message

    const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head><title>${bookTitle}</title><style>body{font-family:'Times New Roman',serif;max-width:800px;margin:0 auto;padding:40px 20px;line-height:1.6;color:#333;}h1{color:#8b7355;border-bottom:2px solid #8b7355;padding-bottom:10px;margin-bottom:30px;text-align:center;}.generated-by{text-align:center;margin-bottom:30px;color:#666;font-style:italic;}.content{text-align:justify;font-size:14px;}b,strong{color:#8b7355;}.footer{margin-top:50px;text-align:center;color:#999;font-size:12px;border-top:1px solid #eee;padding-top:20px;}</style></head>
        <body><h1>${bookTitle}</h1><div class="generated-by">Generated by AutoBrief AI</div><div class="content">${readingContent.innerHTML}</div><div class="footer">Generated on ${new Date().toLocaleDateString()} | AutoBrief.com</div></body>
        </html>
    `;
    ...
    // THIS IS THE NEW PART - IT ADDS ERROR CATCHING
    html2pdf().from(htmlContent).set(opt).save().catch(err => {
      // This will catch any hidden error from the library
      console.error("THIS IS THE REAL PDF ERROR:", err);
      showNotification('PDF generation failed. Please check the console for details.', 'error');
    });
}
